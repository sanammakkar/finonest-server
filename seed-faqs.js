...
const db = {
  host: 'eok0ss8kgwwsc8s8sw8g0840',
  port: 3306,
  user: 'mysql',
  password: 'Root@6378110608#',
  database: 'Fino',
...
async function seed() {
  ...
  await conn.execute(`CREATE TABLE IF NOT EXISTS faqs (
  ...
  try { await conn.execute(`ALTER TABLE faqs ADD COLUMN IF NOT EXISTS page VARCHAR(100) NOT NULL DEFAULT 'home'`); } catch(e) {}

  let inserted = 0, skipped = 0;
  const pageCounters = {};

  for (const [page, question, answer] of faqs) {
    const [rows] = await conn.execute('SELECT id FROM faqs WHERE page = ? AND question = ?', [page, question]);
    if (rows.length > 0) { skipped++; continue; }

    const order = pageCounters[page] ?? 0;
    await conn.execute('INSERT INTO faqs (page, question, answer, sort_order) VALUES (?, ?, ?, ?)', [page, question, answer, order]);
    pageCounters[page] = order + 1;
    inserted++;
    console.log(`✓ [${page}] ${question.substring(0, 50)}...`);
  }
